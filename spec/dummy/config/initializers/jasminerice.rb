...
if defined?(Jasminerice) == 'constant'
  Jasminerice.setup do |config|
    ...
    config.fixture_path = 'spec/javascripts/training_app/fixtures'
  end

  Rails.application.config.assets.paths << TrainingApp::Engine.root.join("spec", "javascripts", "training_app")
  # Add engine to view path so that spec/javascripts/fixtures are accessible
  ActionController::Base.prepend_view_path TrainingApp::Engine.root
end
